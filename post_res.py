...
for attack_name in ['C', 'W']:
    for method in ['latency', 'baseline']:
        ...
        for data_name in MODEL_NAME_LIST:
            ...
            dist_data_path = os.path.join('res', data_name + '_' + attack_name + '_' + method + '.csv')
            latency_res = torch.load(latency_file)
            I_flops, I_cpu, I_gpu = [], [], []
            flops, cpu, gpu = [], [], []

            for data in latency_res:
                I_flops.append((np.array(data['flops'][1:]) / data['flops'][0] - 1).reshape([1, -1]))
                I_cpu.append((np.array(data['cpu'][1:]) / data['cpu'][0] - 1).reshape([1, -1]))
                I_gpu.append((np.array(data['cuda'][1:]) / data['cuda'][0] - 1).reshape([1, -1]))

                flops.append(np.array(data['flops'])[[0, sel_per]].reshape([1, -1]))
        ...
        save_path = os.path.join('res', method + '_' + attack_name + '.csv')
