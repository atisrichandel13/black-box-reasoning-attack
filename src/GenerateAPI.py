...
def generate(
    ...
    if model_kwargs.get("attention_mask", None) is None and requires_attention_mask and accepts_attention_mask:
        # Llama 3.1 Fix: Manually construct the attention mask (1 for real tokens, 0 for padding)
        # This bypasses the HF warning when pad_token == eos_token
        model_kwargs["attention_mask"] = (inputs_tensor != pad_token_id).long()
    ...
    # )
    # 8. prepare stopping criteria
    # Llama 3.1 / Transformers v4.57+ Fix: Initialize missing internal tensor variables
    ...
    if not hasattr(generation_config, '_pad_token_tensor'):
        generation_config._pad_token_tensor = None
