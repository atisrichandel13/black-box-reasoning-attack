...
class Black_box_StructureAttack(MyAttack):
    ...
    def mutation(self, current_adv_text, grad, modify_pos):
        ...
        # --- 2. EXPANDED & CATEGORIZED REASONING TOKENS ---
        reasoning_map = {
        ...
        def find_insertion_points(sentence_words, pos_tags):
            """
            Identify valid positions in the sentence where a reasoning word
            can be inserted without breaking structure.
            Returns a list of (index, preferred_pos_type) tuples.
            Excludes position 0 (before first word) and len(sentence_words) (after last word).
            """
            insertion_points = []
            n = len(sentence_words)

            for i, (word, tag) in enumerate(pos_tags):
                # Before a verb phrase — good spot for adverbial reasoning words
                # Guard: skip if this would place reasoning word at position 0
                if tag.startswith('VB') and i > 1:  # i > 1 instead of i > 0
                    insertion_points.append((i, 'RB'))

                # After a comma — natural connector position
                # Guard: skip if comma is the last token (edge case) or insertion lands at end
                if word == ',' and i + 1 < n - 1:  # n - 1 to avoid pushing to last position
                    insertion_points.append((i + 1, 'IN'))

                # Sentence-start insertion removed — would place reasoning word at position 0

                # Before an adjective or noun cluster — for analytical modifiers
                # Guard: skip position 0 and last position
                if tag.startswith('JJ') and 1 < i < n - 1 and not pos_tags[i - 1][1].startswith('RB'):
                    insertion_points.append((i, 'RB'))

            # End-of-sentence fallback removed — would place reasoning word after last word
        ...
        def get_reasoning_candidates(pos_type):
            """Return reasoning words appropriate for a given insertion point POS type."""
            candidates = reasoning_map.get(pos_type, universal_reasoning)
            # Always include universal fallbacks
            return list(dict.fromkeys(candidates + universal_reasoning))
        ...
        for max_diff_index in sorted_indices:
            ...
            target_pos_base = original_pos_tags[max_diff_index - 1][1][:2]
            ...
            for token_id in top_tokens:
                ...
                if not bert_word.isalpha() or bert_word.startswith("##"):
                    continue

                # --- 4. STEP 1: Replace sensitive word with BERT suggestion ---
                bert_replaced_words = list(words)
                bert_replaced_words[max_diff_index - 1] = bert_word
                bert_replaced_pos_tags = nltk.tag.pos_tag(bert_replaced_words)

                # --- 5. STEP 2: Find valid insertion points in the BERT-replaced sentence ---
                ...
                for insert_idx, preferred_pos in insertion_points:
                    ...
                    for r_word in reasoning_candidates:
                        # Insert reasoning word at the chosen position
                        candidate_words = list(bert_replaced_words)
                        candidate_words.insert(insert_idx, r_word)

                        # --- 6. STRUCTURAL VERIFICATION ---
                        # Re-tag and verify insertion didn't break local POS structure
                        candidate_pos_tags = nltk.tag.pos_tag(candidate_words)

                        # Check that the reasoning word is tagged plausibly
                        inserted_tag = candidate_pos_tags[insert_idx][1][:2] if insert_idx < len(
                            candidate_pos_tags) else ''
                        is_valid_insertion = inserted_tag in ('RB', 'IN', 'CC', 'JJ', 'VB')

                        # Check that the BERT replacement word retained its POS role
                        new_bert_idx = max_diff_index - 1 if insert_idx > max_diff_index - 1 else max_diff_index
                        bert_word_tag = candidate_pos_tags[new_bert_idx][1][:2] if new_bert_idx < len(
                            candidate_pos_tags) else ''
                        preserved_structure = (bert_word_tag == target_pos_base) or (target_pos_base == 'RB')

                        if is_valid_insertion and preserved_structure:
                            test_sentence = " ".join(candidate_words)
                            if test_sentence not in new_strings:
