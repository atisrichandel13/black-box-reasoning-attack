...
class Black_box_StructureAttack(MyAttack):
    ...
    def mutation(self, current_adv_text, grad, modify_pos, used_tokens=None):
        ...
        # Pre-cache reasoning candidates, filtering out already used tokens
        reasoning_cache = {
            pos: [w for w in list(dict.fromkeys(words_list + universal_reasoning)) if w not in used_tokens]
            for pos, words_list in reasoning_map.items()
        }
        reasoning_cache['default'] = [w for w in universal_reasoning if w not in used_tokens]
        ...
        # Pre-compute POS tags and number flags for original sentence
